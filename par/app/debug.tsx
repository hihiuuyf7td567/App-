...
export default function DebugScreen() {
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
      ...
      <Text style={styles.meta}>{courses.length} courses · {courses.reduce((n, c) => n + c.stops.length, 0)} stops · 12 bars seeded</Text>

      {courses.map((course) => (
        <View key={course.id} style={styles.courseCard}>
          <View style={styles.courseHeader}>
            <Text style={styles.courseName}>{course.name}</Text>
            {course.isFeatured && <Text style={styles.featuredBadge}>featured</Text>}
          </View>
          ...
          <View style={styles.stopsTable}>
            <View style={styles.tableHeader}>
              <Text style={[styles.tableCell, styles.tableHeadText, styles.colPos]}>#</Text>
              <Text style={[styles.tableCell, styles.tableHeadText, styles.colName]}>Bar</Text>
              <Text style={[styles.tableCell, styles.tableHeadText, styles.colType]}>Type</Text>
              <Text style={[styles.tableCell, styles.tableHeadText, styles.colPar]}>Par</Text>
            </View>

            {course.stops.map((stop) => (
              <View key={stop.barId} style={styles.tableRow}>
                <Text style={[styles.tableCell, styles.colPos, styles.cellText]}>{stop.position}</Text>
                <Text style={[styles.tableCell, styles.colName, styles.cellText]}>{stop.bar.name}</Text>
                <Text style={[styles.tableCell, styles.colType, styles.cellMuted]}>{stop.bar.type}</Text>
                <Text style={[styles.tableCell, styles.colPar, styles.parNum]}>{stop.bar.baselinePar}</Text>
              </View>
            ))}

            <View style={styles.tableFoot}>
              <Text style={[styles.tableCell, styles.colPos]} />
              <Text style={[styles.tableCell, styles.colName, styles.footText]}>Total</Text>
              <Text style={[styles.tableCell, styles.colType]} />
              <Text style={[styles.tableCell, styles.colPar, styles.footPar]}>{course.coursePar}</Text>
            </View>
          </View>
        </View>
      ))}
...
const styles = StyleSheet.create({
  ...
  screenTitle: { color: "#fff", fontSize: 20, fontWeight: "500", marginBottom: 4 },
  ...
  tableHeader: { flexDirection: "row", paddingVertical: 6, borderBottomWidth: 0.5, borderColor: "#2a2a2a" },
  tableHeadText: { color: "#555", fontSize: 11, textTransform: "uppercase" },
  tableRow: { flexDirection: "row", paddingVertical: 7, borderBottomWidth: 0.5, borderColor: "#1e1e1e" },
  ...
  footPar: { color: "#FAC775", fontSize: 14, fontWeight: "500", textAlign: "right" },
